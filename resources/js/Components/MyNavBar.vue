...
<template>
    <div class="">
        <nav class="bg-transparent relative p-2">
            <div
                class="container-fluid w-full justify-evenly flex place-items-center md:flex flex-wrap md:place-items-center md:justify-between px-2 sm:px-6 md:px-6"
            >
                <!-- Application Logo  -->
                ...
                <div class="" v-show="$page.props.auth.user.role == 'admin'">
                    ...
                    <input
                        @keyup="searchAdmin"
                        id="searchVal"
                        class="sm:h-10 sm:w-96 rounded-md md:h-10 md:w-96 w-36"
                        v-model="form.searchValue"
                        placeholder="Search for anything / anyone"
                        type="text"
                    />
                </div>
                ...
                <div v-show="$page.props.auth.user.role == 'user'">
                    ...
                    <input
                        @keyup="searchUser"
                        id="searchVal"
                        class="sm:h-10 sm:w-96 rounded-md md:h-10 md:w-96 w-40 m-auto"
                        v-model="form.searchValue"
    ...
    <div v-for="user in props.searchedUser" :key="user.id">
        <div
            v-show="
                user.name == form.searchValue || user.email == form.searchValue
            "
        >
